...
<Note>
`WithInstanceId` applies to the **Temporal runtime** only. On the local runtime, agents run in-process with no task queue requirement and instance IDs have no effect.
</Note>
